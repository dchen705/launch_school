...
observations
expect:
  case of 2 documents having term
  expect: only 1 output to be 0
  actual: all outputs are 0

  case of 1 documents having term
    expect: 2 output to be 0
    actual: 2 outputs to be 0.

that makes me think the problem is in the idf method:
I noticed this expression:
Math.log(number_of_documents / number_of_documents_with_term) is doing
integer division possibly - which means if denominator is greater than 1, will truncate to 0. which possibly led to the extra 0 outputs when number of documents greater than 1 having the term inside itself. 

^ Yes, changing to this:
Math.log(number_of_documents / Float(number_of_documents_with_term)
fixed it
can also do:
Math.log(number_of_documents.fdiv(number_of_documents_with_term))

further exploration:
one more issue w/ division - 
is it if we divide by 0? that is no documents have the term

make a ternary. have idf() return 0 if `number_of..` is 0
